...
const ADMIN_CAP = '0xce8c355459dd0f44117608f4a282ab63c70dcba10ca596265e473353bdee435a'; 
...
const MYTHIC_SUPPLY = 21;
const NORMAL_SUPPLY = 6000;
const MINT_PRICE = 0;
...
const DEBUG_START_DELAY = 10000;
const DEBUG_START_TIME = 1755975600000;

const MOCK_COLLECTION_STATE = {
    total_minted: 0,
    mythic_minted: 0,
    normal_minted: 0,
...
interface BeelieversMintProps {}

function BeelieversMintFunction({}: BeelieversMintProps) {
    ...
    const [kioskInfo, setKioskInfo] = useState<{ kioskId: string; kioskCapId: string; address: string; isPersonal?: boolean } | null>(null);
    ...
    }, [DEBUG_MODE, collectionState.mint_start]);
    ...
    useEffect(() => {
        const initializeKioskInfo = async () => {
            if (account) {
                ...
                try {
                    const { kioskOwnerCaps, kioskIds } = await kioskClient.getOwnedKiosks({ address: account.address });
        ...
        if (!DEBUG_MODE) {
            try {
                ...
                if (collectionObject.data?.content && 'fields' in collectionObject.data.content) {
                    const content = collectionObject.data.content.fields as any;
                    console.log(content);
                    setCollectionState({
        ...
        if (!DEBUG_MODE && account) {
            ...
            try {
                ...
                txb.moveCall({
                    ...
                    arguments: [
                        txb.object(COLLECTION_OBJECT_ID),
                ...
                txb2.moveCall({
                    ...
                    arguments: [
                        txb2.object(COLLECTION_OBJECT_ID),
                ...
                txb3.moveCall({
                    ...
                    arguments: [
                        txb3.object(AUCTION_OBJECT_ID),
                        txb3.pure.address(account.address)
                    ]
                });
                ...
                <div className="animate-in slide-in-from-right-4 duration-1000 delay-500 w-full flex justify-center">
                    <div className="w-full lg:w-2/3 xl:w-1/2 space-y-6">
                        ...
                        {isLoadingEligibility ? (
                            ...
                            mintingPhase === 1 && isEligible && !hasMinted && (
                                <Card className="shadow-2xl border-primary/20 hover:border-primary/40 transition-all duration-300 animate-in slide-in-from-bottom-2 duration-700">
                                    <CardContent className="p-6 lg:p-8 rounded-lg text-white flex flex-col w-full gap-6 bg-gradient-to-br from-azure-10 via-azure-15 to-azure-20">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-muted-foreground">
                                                        {isPartner ? "Partner mint available" : "Auction winner mint available"}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div className="flex flex-col gap-4 lg:gap-6 py-0 w-full lg:text-base leading-relaxed">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="text-center p-4 bg-primary/10 rounded-lg border border-primary/20">
                                    <div className="text-2xl font-bold text-primary">{collectionState.total_minted}</div>
                                    <div className="text-sm text-muted-foreground">Total Minted</div>
                                </div>
                                <div className="text-center p-4 bg-primary/10 rounded-lg border border-primary/20">
                                    <div className="text-2xl font-bold text-primary">{collectionState.mythic_minted}</div>
                                    <div className="text-sm text-muted-foreground">Mythic NFTs</div>
                                </div>
                                <div className="text-center p-4 bg-primary/10 rounded-lg border border-primary/20">
                                    <div className="text-2xl font-bold text-primary">{TOTAL_SUPPLY - collectionState.total_minted}</div>
                                    <div className="text-sm text-muted-foreground">Remaining</div>
                                </div>
                            </div>

                            <p>
                                BTCFi Beelievers NFT is a free mint for partners and auction winners. 
                                Each eligible address can mint one NFT. Partners have a chance to mint 
                                rare mythic NFTs while auction winners receive normal NFTs.
                            </p>
                        </div>
                    </CardContent>
                </Card>
