...
describe("hexToUint8Array", () => {
	...
	test("throws error for non-hex characters", () => {
		expect(() => hexToUint8Array("fff")).toThrow("even number of characters");
		// Test an even-length string with non-hex characters
		expect(() => hexToUint8Array("xy")).toThrow("Invalid hex character");
		expect(() => hexToUint8Array("12g3")).toThrow("Invalid hex character");
	});

	test("ignores 0x", () => {
		const result = hexToUint8Array("0x0001");
